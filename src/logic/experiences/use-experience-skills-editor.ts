import type { ExperienceSkillDraft } from '@/model/experience'

export function useExperienceSkillsEditor (options: {
  props: {
    modelValue: string[]
    disabled: boolean
  }
  ...
  watch(
    ...
    next => {
      ...
      skillsDraft.value = normalized.map(skill_name => ({
        id: null,
        skill_name,
      }))
    },
  ...
  function emitModelFromDraft (): void {
    ...
    options.emit('update:modelValue', skillsDraft.value.map(item => item.skill_name))
  ...
  function saveSkill (): void {
    ...
    const existing = skillsDraft.value.map(s => s.skill_name.trim().toLowerCase())
